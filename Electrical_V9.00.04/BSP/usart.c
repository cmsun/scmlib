...
__weak void USART2_Callback(uint16_t data)
{
    (void)data;
}
...
void USART2_IRQHandler(void)
{
    if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)
    {
        ...
        USART2_Callback(RecByte);
    }
}

/*
 * //定义成弱函数后可以在其它地方被同名函数重载。
 * #define DECLARE_FUNCTION_USARTx_Callback(USARTx) \
 * __weak void USARTx##_Callback(uint16_t data) \
 * { \
 *     (void)data; \
 * } \
 * 
 * #define DECLARE_FUNCTION_USARTx_IRQHandler(USARTx) \
 * void USARTx##_IRQHandler(void) \
 * { \
 *     if(USART_GetITStatus(USARTx, USART_IT_RXNE) != RESET) \
 *     { \
 *         uint16_t RecByte = USART_ReceiveData(USARTx); \
 *         USARTx##_Callback(RecByte); \
 *     } \
 * }
 * 
 * DECLARE_FUNCTION_USARTx_Callback(USART2);
 * DECLARE_FUNCTION_USARTx_IRQHandler(USART2);
 */
